...
using Ciribob.DCS.SimpleRadio.Standalone.Client.Network.CODCSAirspace.Models;
...
namespace Ciribob.DCS.SimpleRadio.Standalone.Client.Network.CODCSAirspace;

public class CODCSAirspaceSyncHandler
{
    ...
    private UdpClient _CODCSAirspacePositionListener;
    private volatile bool _stop;

    public CODCSAirspaceSyncHandler(string guid)
    {
        _guid = guid;
        _clientStateSingleton = ClientStateSingleton.Instance;

        _heightOffset = _globalSettings.GetClientSetting(GlobalSettingsKeys.CODCSAirspaceHeightOffset).DoubleValue;
    }
        ...
        Task.Factory.StartNew(() =>
        {
            while (!_stop)
                try
                {
                    var localEp = new IPEndPoint(IPAddress.Any,
                        _globalSettings.GetNetworkSetting(GlobalSettingsKeys.CODCSAirspaceIncomingUDP));
                    _CODCSAirspacePositionListener = new UdpClient(localEp);
                    break;
                }
                catch (Exception ex)
                {
                    Logger.Warn(ex,
                        $"Unable to bind to the Combined Ops DCS Airspace Export Listener Socket Port: {_globalSettings.GetNetworkSetting(GlobalSettingsKeys.CODCSAirspaceIncomingUDP)}");
                    Thread.Sleep(500);
                }
            ...
            while (!_stop)
                try
                {
                    ...
                    var bytes = _CODCSAirspacePositionListener.Receive(ref groupEp);

                    var CODCSAirspacePositionWrapper = JsonSerializer.Deserialize<CODCSAirspaceMessageWrapper>(
                        Encoding.UTF8.GetString(bytes, 0, bytes.Length),
                            new JsonSerializerOptions() { IncludeFields = true }
                        );

                    if (CODCSAirspacePositionWrapper != null)
                    {
                        if (CODCSAirspacePositionWrapper.los != null)
                            HandleLOSResponse(CODCSAirspacePositionWrapper.los);
                        else if (CODCSAirspacePositionWrapper.controller != null)
                            HandleCODCSAirspaceUpdate(CODCSAirspacePositionWrapper.controller);
                    }
                }
                catch (SocketException e)
                {
                    if (!_stop) Logger.Error(e, "SocketException Handling Combined Ops DCS Airspace UDP Message");
                }
                catch (Exception e)
                {
                    Logger.Error(e, "Exception Handling Combined Ops DCS Airspace UDP Message");
                }

            try
            {
                _CODCSAirspacePositionListener.Close();
            }
            catch (Exception e)
            {
                Logger.Error(e, "Exception stoping Combined Ops DCS Airspace UDP listener");
            }
        });

        StartCODCSAirspaceLOSSender();
    }

    private void HandleCODCSAirspaceUpdate(CODCSAirspaceMessageWrapper.CODCSAirspacePosition controller)
    {
        _clientStateSingleton.CODCSAirspaceLastReceived = DateTime.Now.Ticks;
        ...
        if (_clientStateSingleton.ShouldUseCODCSAirspacePosition())
        {
    ...
    private void HandleLOSResponse(CODCSAirspaceMessageWrapper.CODCSAirspaceLineOfSightResponse response)
    {
        SRClientBase client;
    ...
    private void StartCODCSAirspaceLOSSender()
    {
        var _udpSocket = new UdpClient();
        var _host = new IPEndPoint(IPAddress.Loopback,
            _globalSettings.GetNetworkSetting(GlobalSettingsKeys.CODCSAirspaceOutgoingUDP));
        ...
        Task.Factory.StartNew(() =>
        {
            using (_udpSocket)
            {
                while (!_stop)
                {
                    try
                    {
                        if (_clientStateSingleton.IsCODCSAirspaceConnected)
                        {
                    ...
                    {
                        Logger.Error(e, "Exception Sending Combined Ops DCS Airspace LOS Request Message");
                    }
                ...
                {
                    Logger.Error(e, "Exception stoping Combined Ops DCS Airspace LOS Sender ");
                }
            }
        });
    }

    private List<CODCSAirspaceLineOfSightRequest> GenerateDcsLosCheckRequests()
    {
        var clients = _clients.Values.ToList();

        var requests = new List<CODCSAirspaceLineOfSightRequest>();
        ...
        if (_clientStateSingleton.PlayerCoaltionLocationMetadata.LngLngPosition != null
            ...
            && _clientStateSingleton.ShouldUseCODCSAirspacePosition()
            && _serverSettings.GetSettingAsBool(ServerSettingsKeys.LOS_ENABLED))
            foreach (var client in clients)
                ...
                if (client.LatLngPosition != null
                    ...
                    requests.Add(new CODCSAirspaceLineOfSightRequest
                    {
